    ...
    if activity is not None:
        ...
        if name == 'globals':
            obj = bs.newNode('globals')
                      ...
                      'footingMaterial', 'attackMaterial', 'footingMaterialPlayer']:
            obj = bs.Material()
            ...
            obj = bs.Material()
        elif name == 'fireMaterial':
            obj = bs.Material()
            obj.addActions(('message', 'theirNode', 'atConnect', bs.FireMessage()))
...
def animateArrayFor(node, attr, size, keys, loop=False,
                 offset=0, driver='gameTime'):
    """
    category: Game Flow Functions
...
class Spawner(object):
    ...
    def __init__(self, data=None, pt=(0, 0, 0), spawnTime=1000,
        ...
        self._light2 = bs.newNode('flag', attrs={
            'position':tuple(pt),
            'radius':0.1,
            'colorTexture':bs.getTexture('cuteSpaz'),
            'lightVolumes':False})
        self._light = bs.newNode('light', attrs={
            'position':tuple(pt),
            'radius':0.1,
            'color':(random.random()*10,random.random()*10,random.random()*10),
            'lightVolumes':False})
       # portalObjects.SpawnerM(position=self.node.position)
    ...
    def _spawn(self):
        ...
        bs.gameTimer(400, self._light2.delete)
...
class Spawner2(object):
    ...
    def __init__(self, data=None, pt=(0, 0, 0), spawnTime=1000,
        ...
        self._light = bs.newNode('light', attrs={
            'position':tuple(pt),
            'radius':0.0, #you can add radius if you want
            'color':(1,0,0),
            'lightVolumes':False})
       # portalObjects.SpawnerM(position=self.node.position)
        s = float(spawnTime)/3750
...
class Background(bsGame.Actor):
    ...
    def __init__(self, fadeTime=500, startFaded=False, showLogo=False, background='black', logo='star'):
        ...
        with bs.Context(session):        	
            self.node = bs.newNode('image', delegate=self, attrs={
                'fillScreen':True,
                'texture':bs.getTexture(random.choice(['black','reflectionSharp_-x','reflectionChar_-z','reflectionChar_+z','lightSoft','shield','sparks'])), #
                'tiltTranslate':-0.3,
                'hasAlphaChannel':False,
                'color':(1,0,0)})
            bs.animateArray(self.node,'color',3,{0:(1,0,0),500:(0.80,0,0),1000:(1,0,0),1500:(0.89,0,0),2000:(1,0,0),2500:(0.79,0,0),3000:(1,0,0)},loop=True)  if random.random() > 0.5 else bs.animateArray(self.node,'color',3,{0:(1,1,1),500:(0.80,0.80,0.80),1000:(1,1,1),1500:(0.89,0.89,0.89),2000:(1,1,1),2500:(0.79,0.79,0.79),3000:(1,1,1)},loop=True) if random.random() > 0.5 else bs.animateArray(self.node,'color',3,{0:(0,0,2),500:(0,2,0),1000:(2,0,0),1500:(2,2,0),2000:(2,0,2),2500:(0,1,6),3000:(1,2,0)},loop=True)
            bs.animate(self.node, 'opacity', {0: 0.0, 200: 1.0},loop=False) if random.random() > 0.5 else bs.animate(self.node, 'opacity', {0: 1.0, 500: 0,1000: 1.0, 1500: 0, 2000:1},loop=True)
            bs.animate(self.node, 'rotate', {0: 0.0, 1000:0}, loop=False) if random.random() > 0.5 else bs.animate(self.node, 'rotate', {0: 0.0, 2000: 360.0}, loop=True) if random.random() > 0.5 else bs.animate(self.node, 'rotate', {0: 0.0, 500: 260.0, 1000:-260.0, 1500:0}, loop=True)
            if not startFaded:
            ...
            if showLogo:
                ...
                self.logo = bs.newNode('image', owner=self.node, attrs={
                    'texture':bs.getTexture(random.choice(['star','heart','folder','graphicsIcon','buttonBomb','achievementEmpty','achievementOutline','chestIcon','gameCircleIcon','replayIcon','tv'])),
                ...
                bs.animateArray(self.logo,'color',3,{0:(0,0,2),500:(0,2,0),1000:(2,0,0),1500:(2,2,0),2000:(2,0,2),2500:(0,1,6),3000:(1,2,0)},loop=True) if random.random() > 0.5 else bs.animateArray(self.logo,'color',3,{0:(1,1,1),500:(0.80,0.80,0.80),1000:(1,1,1),1500:(0.89,0.89,0.89),2000:(1,1,1),2500:(0.79,0.79,0.79),3000:(1,1,1)},loop=True)
                bs.animate(self.logo, 'opacity', {0: 0.0, 500: 1.0,1000: 1.0, 1500: 0},loop=True) if random.random() > 0.3 else bs.animate(self.logo, 'opacity', {0: 0.0, 200: 1.0,300: 1.0, 400: 0},loop=True) if random.random() > 0.3 else bs.animate(self.logo, 'opacity', {0: 0.0},loop=False) if random.random() > 0.3 else bs.animate(self.logo, 'opacity', {0: 1.0},loop=False)
                bs.animate(self.logo, 'rotate', {0: 0.0, 2000: 360.0}, loop=True) if random.random() > 0.3 else bs.animate(self.logo, 'rotate', {0: 0.0}, loop=False) if random.random() > 0.5 else bs.animate(self.logo,'rotate', {0: 0.0, 500: 260.0, 1000:-260.0, 1500:0}, loop=True)
                ...
                if not bs.getEnvironment()['vrMode']:
                    ...
                    for attr in ['input0', 'input1']:
                        animate(self.c, attr, {0:0.693, 50:0.8, 500:0.693},
                                loop=True)
                    self.c.connectAttr('output', self.logo, 'scale')
                    c = bs.newNode('combine', owner=self.node, attrs={'size':2.5})
                    c.connectAttr('output', self.logo, 'position')
                    # gen some random keys for that stop-motion-y look
                    keys = {}
                    time = 0
                    for i in range(10):
                        keys[time] = (random.random()-0.5)*-0.15
                        time += random.random() * 300
                    animate(c, "input0", keys, loop=True)
                    keys = {}
                    time = 0
                    for i in range(10):
                        keys[time] = (random.random()-0.5)*0.4015 + 0.05
...
class TipsText(bsGame.Actor):
    ...
    def __init__(self, offsY=100):
        bs.Actor.__init__(self)
        self._tipScale = 1.0
        self._tipTitleScale = 1.0
        self._offsY = offsY
        self.node = bs.newNode('text', delegate=self, attrs={
            'text':'',
            'scale':self._tipScale,
            'hAlign':'left',
            'color':(random.random()*1,random.random()*1,random.random()*1),
            'maxWidth':800,
            'vrDepth':-20,
            'vAlign':'center',
            'vAttach':'bottom'})
        self.titleNode = bs.newNode('text', delegate=self, attrs={
            'text':random.choice(['Dato:','Sugerencia:','Dato Curioso:','Dato Interesante:']), #ODIO LOS REPETITIVO
        ...
        self._changeTimer = bs.Timer(self._messageDuration+self._messageSpacing,
                                     ...
                                     repeat=True)
        self._combine = bs.newNode("combine", owner=self.node, attrs={
            'input0':1.0, 'input1':0.8, 'input2':1.0, 'size':1})
        self._combine.connectAttr('output', self.node, 'color')
    ...
    def changePhrase(self):
        ...
        self.node.text = random.choice(['Baja Los Graficos Si No Adquieres De Minimo De 3 Ram','Cuando Pelees Con Un Jefe Procura No Acercarte Mucho','Yuro Es Buena?, Lo Sabremos En El Capitulo 5','Lleva 3 Anios En Desarrollo Este Modpack En Un Futuro Estara En Unity','Suscribete A Mi Canal Se Llama Catlight','Todos Los Mapas Tienen Extras y Ciclos dia y noche','En Algun Momento Podremos Parar El Virus Que Corrompio Bombsquad?','Todos Los Personajes Tienen Habilidades y Hay Secretos tambien!','No hay tiempo para explicar!','No time to explain!','Hay aue detener a darklight antes que todo empeore!'])
...
class OnScreenCountdown(bsGame.Actor):
    ...
    def __init__(self, duration, endCall=None):
        ...
        self.node = bs.newNode('text', attrs={
            ...
            'scale':1.4,
            'text':''})
     #   bs.animateArray(self.node,'color',3,{0:(0,0,2),500:(0,2,0),1000:(2,0,0),1500:(2,2,0),2000:(2,0,2),2500:(0,1,6),3000:(1,2,0),4500:(0,0,2),5000:(0,2,0), 6500:(2,0,0),7000:(2,2,0), 7500:(2,0,2),8000:(0,1,6), 9500:(1,2,0)},loop=True)
...
class ZoomText(bsGame.Actor):
    ...
    def __init__(self, text, position=(0, 0), shiftPosition=None,
                 ...
                 hAlign="center", color=(random.random()*1,random.random()*1,random.random()*1), jitter=10.0,
                 trailColor=(random.random()*1,random.random()*1,random.random()*1), scale=1.0, projectScale=1.0,
                 tiltTranslate=0.0, maxWidth=None, lifespan2=3000):
        bs.Actor.__init__(self)
        self._dying = False
        ...
        if lifespan is not None:
            bs.gameTimer(lifespan2, bs.WeakCall(self.handleMessage,
                                               bs.DieMessage()))
...
def _haveProOptions():
    ...
    return True if (
        bsInternal._getAccountMiscReadVal2('proOptionsUnlocked', True)
        or bs.getConfig().get('lc14292', 0) > 1) else True
...
def _configServer():
    ...
    _setLanguage(config.get('language', 'Spanish'))

    bs.getConfig()['Auto Balance Teams'] = config.get('autoBalanceTeams', True)

    bsInternal._setPublicPartyMaxSize(config.get('maxPartySize', 90000))
    bsInternal._setPublicPartyName(config.get('partyName', 'party'))
    bsInternal._setPublicPartyStatsURL(config.get('statsURL', 'https://discord.com/invite/RCXjrHqAhc'));
...
def _runServer():
    ...
    # set series lengths
    bsTeamGame.gTeamSeriesLength = config.get('teamsSeriesLength', 24)
    bsTeamGame.gFFASeriesLength = config.get('ffaSeriesScoreToWin', 35)
